...
export const App = () => {
	const [activeTab, setActiveTab] = useState<TabType>(Tabs.TODOS);
	...
	const onToggleTodo = (id: number) => {
		const updatedTodos = todos.map((todo: Todo) => (todo.id === id ? { ...todo, done: !todo.done } : todo));
	...
	const onToggleFavorite = (id: number) => {
		const updatedTodos = todos.map((todo: Todo) => (todo.id === id ? { ...todo, favorite: !todo.favorite } : todo));
	...
	return (
		<div className="flex flex-col min-h-screen bg-background text-text">
			<Navbar brand="To-Do App" activeTab={activeTab} setActiveTab={setActiveTab} />
